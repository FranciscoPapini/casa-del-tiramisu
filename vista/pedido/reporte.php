...
if($_GET['id']) {
      ...
      class PDF extends FPDF
      {
          function Header()
          {
            ...
            $this->Cell(30);
            $this->Cell(130, 10, 'Detalle de Pedido', 0, 0, 'C');
  ...
  $pdf->Cell(20, 6, 'Fecha de Pedido:', 0, 0, 'R', 0);
  
  $fecha = Util::dbToDate($pedido->getFecha());
  $pdf->SetFont('Helvetica', 'B', 10);
  $pdf->Cell(70, 6, $fecha, 0, 0, 'L', 0);
  ...
  $pdf->SetFont('Helvetica', 'B', 10);
  $pdf->Cell(20, 6, utf8_decode($encargado), 0, 1, 'L', 0);

  $pdf->SetX(35);
  $pdf->SetFont('Helvetica', '', 10);
  $pdf->Cell(20, 6, 'Cliente:', 0, 0, 'R', 0);
  
  $nombre = ucwords($cliente->getNombre());
  $pdf->SetFont('Helvetica', 'B', 10);
  $pdf->Cell(80, 6, utf8_decode($nombre), 0, 1, 'L', 0);

  $pdf->SetX(35);
  $pdf->SetFont('Helvetica', '', 10);
  $pdf->Cell(20, 6, utf8_decode("Teléfono:"), 0, 0, 'R', 0);
  
  $pdf->SetFont('Helvetica', 'B', 10);
  $pdf->Cell(20, 6, $cliente->getTelefono(), 0, 1, 'L', 0);

  $pdf->SetX(35);
  $pdf->SetFont('Helvetica', '', 10);
  $pdf->Cell(20, 6, utf8_decode("Descripción:"), 0, 0, 'R', 0);
  
  if($pedido->getDescripcion() == '0') { $descripcion = '-'; } else { $descripcion = ucfirst($pedido->getDescripcion()); }
  $pdf->SetFont('Helvetica', 'B', 10);
  $pdf->Cell(120, 6, utf8_decode($descripcion), 0, 1, 'L', 0);

  $pdf->SetX(35);
  $pdf->SetFont('Helvetica', '', 10);
  $pdf->Cell(20, 6, utf8_decode("Envío:"), 0, 0, 'R', 0);
  
  if($pedido->getEnvio() == 0) { $env = 'No'; } else { $env = 'Si'; }
  $pdf->SetFont('Helvetica', 'B', 10);
  $pdf->Cell(20, 6, $env, 0, 1, 'L', 0);
 
  if($pedido->getEnvio() == 1) {

  $pdf->SetX(35);
  $pdf->SetFont('Helvetica', '', 10);
  $pdf->Cell(20, 6, utf8_decode("Dirección de Envío:"), 0, 0, 'R', 0);
  
  if($pedido->getDireccion() == '0') { $direcc = '-'; } else { $direcc = ucwords($pedido->getDireccion()); }
  $pdf->SetFont('Helvetica', 'B', 10);
  $pdf->Cell(120, 6, utf8_decode($direcc), 0, 1, 'L', 0);
  ...
  $pdf->SetX(7);
  $pdf->SetFont('Helvetica', '', 9);
  $pdf->Cell(163, 6, 'Envio', 1, 0, 'R', 0);

  $pdf->SetFont('Helvetica', '', 9);
  $prec = '$ ' . number_format($precio, 2);
  $pdf->Cell(30, 6, utf8_decode($prec), 1, 1, 'L', 0);
